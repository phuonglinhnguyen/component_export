...
import { getDataExport } from './../../providers/faKedata/export_configuration'
...
const ExportComponent: React.FC<IDefautProps, IDefautState> = (props) => {
	...
	const [selectedExportConfig, setSelectedExportConfig] = useState(null);
	const [strSearch, setStrSearch] = useState(null);
	const [page, setPage] = useState(0);
	const [rowsPerPage, setRowsPerPage] = useState(10);
	const [configDel, setConfigDel] = useState('');
	const [isOpenAdd, setIsOpenAddDialog] = useState(false);
	const [isOpenView, setIsOpenViewDialog] = useState(false);
	const [isOpenDel, setIsOpenDelDialog] = useState(false);
	// =====Search
	...
	return (
		<React.Fragment>
			<div className={classes.container}>
				...
				/>
				<AddDialogExport isOpen={true || isOpenAdd} setIsOpen={setIsOpenAddDialog} exportConfigs={exportConfigs} setExportConfigs={setExportConfigs} />
				<ViewDialog
					exportConfigs={exportConfigs}
					setExportConfigs={setExportConfigs}
